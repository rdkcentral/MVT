...
```
- Add new URL parameter `testnames` to avoid breaking link to test case, when its number will change.
...
```
Change for live content in seek. After this change we are making seek only backwards, not forward.
```
#
# v2.5.0 (2023-06-22)
...
```
- `DASH PlayReady 4.0 CBCS` test added [ONEM-25815].
- `HLS.js` player default version upgrade from 1.3.0 to 1.4.5 [ONEM-31111].
- `Dash.js` player default version upgrade from 4.4.0 to 4.7.0.
- Set `HLS.MP2TS_AVC_AAC` as mandatory [ONEM-28092].
- Libraries import moved from script in 'body' to the 'head' section in index.html.
```
#
# v2.4.1 (2023-02-24)
...
After: player engine is loaded only when needed, for example in testsuite `dash-dashjs-test` only DASH.js engine will be loaded.
...
```
Reason: it is causing problems with dash-html5 testsuite, streams are not available for tests (error 403).
```
...
- Additional wait before running the tests to fix player engine not loaded completely.
```
#
# v2.1.0 (2022-08-03)
...
```
New page created with a link on the main page "Test any player".
On the `/anyplayer/` subpage user can test any version of available players.
Player version can be changed via remote controller on the Set Top Box or in the URL.
...
Current approach also gives us much greater control when it comes tests selection and parametrization (e.g. now we can mark some tests unstable on a given test suite).
...
```
When the selected player version is different than default one, version be visible in the URL.
```
#
# v1.0.2 (2022-07-05)
### [ [88db535](https://github.com/rdkcentral/MVT/commit/88db535) ] Rename `hardware` attribute into `profile`
```
Profile basically defines user-agent's capabilities and influences the subset of tests visible in the application.
|hardware| name was quite confusing, as the capabilities depend on both device and software running the MVT.
...
```
Created new folder `media` and moved there all bash scripts for creating media assets.
```
#
### [ [e8954ac](https://github.com/rdkcentral/MVT/commit/e8954ac) ] [ONEM-26996] Unify hls/fmp4_multiaudio video segment size
```
This change sets more constraints on the stream generation to make sure that video segments are similar in size.
```
#
# v1.0.0 (2022-06-09)
## INITIAL RELEASE
